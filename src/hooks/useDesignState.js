...
export const useDesignState = () => {
  ...
  const addArt = useCallback((artData) => {
    const art = {
      ...
      position: [0.1, 0.1, 0.01], // 设置初始Z坐标略微大于0，避免Z-fighting
    ...
    updateDesignState(prev => {
      if (elementType === 'art') {
        return {
          ...
          artElements: prev.artElements.map(art => {
            if (art.id === elementId) {
              // rotation 存储为 [x, y, z] 欧拉角 (弧度)
              const currentRotation = art.rotation || [0, 0, 0];
              const flipFactor = Math.PI; // 180度

              let newRotation = [...currentRotation];

              // 仅支持Z轴翻转（绕Z轴旋转180度，实现镜像效果）
              // 绕Z轴旋转180度
              newRotation[2] += flipFactor;
              // 确保角度在 [0, 2*PI] 范围内
              newRotation[2] %= (2 * Math.PI);

              return { ...art, rotation: newRotation };
            }
            return art;
          })
        };
